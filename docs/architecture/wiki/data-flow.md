...
sources:
  ...
  - crates/sesdb-engine/src/daemon.rs 63cae68156de Writer watch_tick run
...
## SESDB Memory（CLI / Console review 入口）

`sesdb memory` 是 evidence-linked durable memory 的唯一写入口；Skill 只读 approved/active。

1. **候选**：显式 propose 携带 content、scope 与至少一个 evidence seq；daemon 校验 seq 确实指向 L1 native evidence，再追加 candidate revision，candidate 不进入 FTS。
2. **审阅**：Console 用只读 cookie 展示 candidate、scope 和 source evidence，只复制 bearer-only CLI 命令，不直接执行管理操作。
3. **批准/撤销**：CLI 提交 ID 与 expected revision；daemon 拒绝 stale revision 或非法状态转换，向 L1 追加 approve/revoke revision，再由 sidecar 投影当前状态。
4. **默认读取**：`sesdb memory list` 与 Skill 只返回 approved/active；rebuild 从 L1 逐 revision 重放，revoked 与 candidate 不会混入默认结果。
